...
export default function Landfill() {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      // For example:
      // await axios.post('/api/endpoint', { ...values, timespan });
    ...
    <div className="w-[22rem] bg-white p-4 rounded-sm border border-gray-200">
      ...
      <div className="flex flex-col gap-3 mt-4">
        <Form layout="vertical" form={form} onFinish={onFinish}>
          ...
          >
            <Select placeholder="Select an Employee">
              {contractEmployees.map((contractEmployee) => (
                <Select.Option
                  key={contractEmployee._id}
                  value={contractEmployee._id}
                >
          ...
          <Form.Item label="Operational Timespan" style={{ marginBottom: 10 }}>
            <Form.Item
              name="timespan"
              noStyle
              rules={[
            ...
            >
              <Input.Group compact style={{ display: "inline-block" }}>
                ...
                <span style={{ padding: "0 8px", marginBottom: "8px" }}>
                  to
                </span>
                <Form.Item
